...
- Edits to `agents/` and `skills/` are edits to the method: do them in the `om-config` session with `/update-method`, which lints with `scripts/lint-method` and records why in `docs/method-ard.md`.
