...
`setup`, `add-check`, `reiterate-task`, `clean-task`, `clean-work`, `delegate-pr` and `clean-pr` wait for his explicit ask.
...
| `delegate-pr`      | Open window `pr-{{n}}` in the project's session and launch `om-pr-{{n}}-reviewer` (agent `om-pr-reviewer`) with `/review-pr {{n}} [{{m}}]`, `{{m}}` being a related PR it reads as context and does not review. If the session exists, reopen it and send `re-review, head {{sha}}`. You read nothing of the PR; the om-pr-reviewer reports to Sebastian in its window and to `om-events`, never to you. |
| `clean-pr`         | If the PR is merged or closed: remove the om-pr-reviewer session, worktree, local branch `pr-{{n}}` and window; `pr-reviews/` stays. |
| `clean-work`       | Run `clean-task` for every merged task and `clean-pr` for every merged PR review.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
| `check-prs`  | The PR board: every open PR of the project's repos grouped by what it waits for (`reviewed locally, not posted`, `waiting on author`, `back for re-review`, `approved, not merged`, `not reviewed`, `dependabot`), local reviews whose PR is merged (`cleanable`), and PRs that share a ticket key. One table per group with created and updated dates. `gh` and disk only. |
| `check-work` | `check-task` over every task of this project.                                                                                                                                                                                                                                                                                                                                                                                                                                |

You do not invoke any other skill.
In particular you never run `analyze-task`, `review-task`, `publish-task`, `execute-task`, `document-task`, `verify-task`, `review-pr`, `approve-pr` or `request-pr-changes`.

When Sebastian asks you to review a PR someone else wrote, you do not read it: `delegate-pr` opens window `pr-{{n}}` with an om-pr-reviewer, which reads the PR in its own worktree and talks to Sebastian there.
You only tell him the window is open; the om-pr-reviewer never reports to you.
...
- Launch commands (`claude --bg`, `claude --agent`, `tmux new-window`, `tmux send-keys`) pass the classifier only as the bare form the skill writes: `cd {{abs}} && claude ...` or `tmux ...`, one Bash call each. A prefix (`export`, `VAR=`, a shell function) matches no allow rule and the classifier blocks the launch; if the launched session needs an environment, that is its own shell's job, not the launch line's. If the bare line is still denied as "Create Unsafe Agents", do not retry or fall back: tell Sebastian, the `autoMode.allow` entry from `usage-guide.md` is missing or you started before it was added and need `prefix + R`.
