...
## 2026-09-24: Every session runs in auto mode, pinned on the launch line; no session is bypassed

- Decision: every launch line carries `--permission-mode auto` and no bypass flag: `consolidate-task` and `start-task` (`claude --bg --permission-mode auto --agent ...` and the fallback forms), `delegate-pr`, `scripts/resume-project` for the om-manager, `bin/resume-overmind` for the cockpit. `permissionMode: bypassPermissions` leaves the frontmatter of `om-reviewer`, `om-developer`, `om-pr-reviewer` and `om-events`. `scripts/install` drops the bypass-disclaimer probe and the `autoMode.allow` check; `usage-guide.md` requires auto mode on the account instead. `docs/02-orchestration.md` states the rule and why; `docs/06-pilot.md` marks the bypass assumption as never true.
  Supersedes: 2026-08-31 "The bypass-permissions disclaimer is a machine requirement", 2026-08-31 "The bypass acceptance check is functional", 2026-08-31 "om-events runs bypassed", and 2026-09-24 "The launch needs an autoMode.allow entry".
- Alternatives rejected: real bypass with `--dangerously-skip-permissions` on every role (keeps the "Create Unsafe Agents" fight with the launcher's classifier, and would need the cockpit and the om-managers bypassed too so the message classes match, which removes the classifier from the sessions Sebastian talks to); `crossSessionInbound: accept` in settings to deliver held messages across classes (treats the symptom, leaves the mode of unattended sessions to a startup race); keeping the frontmatter `permissionMode` as documentation (it is a claim the harness does not honor for `--agent` sessions, and it misled every reader of the method for a month).
- Reason: the transcripts of every om session since 2026-09-01 record `auto` from the first turn: the frontmatter `permissionMode` does not apply to a `--agent` session and `--allow-dangerously-skip-permissions` only adds bypass to the Shift+Tab cycle. The pipeline worked because both ends of every conversation were in the same class. On 2026-09-24 two om-developers (`0048` in auvral on 2.1.281, `0001` in bseen as the first session on 2.1.282) started in manual, the built-in default when the feature flags are late, and Sebastian cycled one of them into bypass by hand; from then on the om-reviewer's messages to it were held for approval, since a bypassed receiver holds messages from a non-bypassed sender. The same morning the launcher's classifier had denied the bypass flag itself as "Create Unsafe Agents". Pinning `auto` removes both failures and the flag that caused the third.
- Debt created: the classifier now applies to om-reviewer and om-developer on the record, not only in fact; no denial has been observed in a month of running that way, but a project whose pipeline needs an action the classifier blocks has no per-agent escape besides `autoMode.allow`. A session Sebastian cycles by hand into bypass silently stops receiving; `check-task` does not detect it.
- Revisit when: an om-developer or om-reviewer reports a classifier denial mid pipeline; or Claude Code lets an agent file pin the mode of a `--agent` session, which would move the flag out of the launch lines.
- Files: agents/om-reviewer.md, agents/om-developer.md, agents/om-pr-reviewer.md, .claude/agents/om-events.md, skills/consolidate-task/SKILL.md, skills/start-task/SKILL.md, skills/delegate-pr/SKILL.md, agents/om-manager.md, docs/02-orchestration.md, docs/06-pilot.md, docs/usage-guide.md, scripts/install, scripts/resume-project, bin/resume-overmind, docs/method-ard.md.
