...
El nombre es deliberadamente distinto de "manager" para que nunca se confunda con el de un proyecto.

### Regla

La cabina observa, agrega, notifica y lleva a Sebastian al lugar correcto.
Nunca planea ni decide sobre un proyecto.
Las conversaciones de producto, arquitectura y técnicas siguen siendo con el manager de cada proyecto, en su sesión de tmux.

Razón: el manager vale por su contexto profundo de un proyecto.
...
El planning con el manager es la conversación de mayor valor del flujo y no se intermedia.
...
- Estado agregado: `check-work` de cada proyecto registrado, en una sola tabla.
  ...
  Sale de disco (`docs/tasks/*.md`), git y `gh`; no le pregunta a los managers.
- Eventos: los managers le avisan por `SendMessage` cuando algo requiere de Sebastian (PR listo, task trabada en delegación).
...
tmux has-session -t "$name" 2>/dev/null || \
  tmux new-session -d -s "$name" -c "$path" -n manager "claude --agent manager -n ${name}-manager"
...
- Cuando un todo se convierte en trabajo para agentes, Sebastian salta al manager del proyecto, lo planea con `plan-task` y lo cierra con `complete-todo`.
...
3. `resume-project {{name}}` lo lleva al manager de ese proyecto.
4. Habla con el manager: aprueba PRs, planea tasks nuevas, delega.
