...
| `--allow-dangerously-skip-permissions` with `--bg` leaves om-reviewer and om-developer without the classifier's blocks | consolidate, start-task | failed, never true: every session ran in auto from the start (the flag only adds bypass to the mode cycle, the frontmatter does not apply to `--agent` sessions); 2026-09-24 all launches pin `--permission-mode auto` and drop the flag |
| The launcher's allow rules cover the launch line as the skill writes it (bare `cd && claude`, bare `tmux`) | consolidate, start-task, delegate-pr | failed 2026-09-20 in auvral: a `export PATH=...; VAR=...` prefix sent the line to the classifier ("Create Unsafe Agents"); fixed by forbidding prefixes in the skills. Failed again 2026-09-24 in bseen on 2.1.281 with the bare line: the classifier judged the bypass flag; resolved by dropping the flag, since the sessions never ran bypassed |
