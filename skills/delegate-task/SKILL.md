...
Output: the reviewer running in its tmux window with `delegated, start` delivered.
From here the manager does not intervene until the reviewer reports `PRs ready`.
...
| Lost | none of the above | launch a new reviewer as `consolidate-task` step 4 does; its `analyze-task` finds `Context & decisions` written and does not ask; wait for `consolidated` |
...
- Never launch a developer; never message the developer.
